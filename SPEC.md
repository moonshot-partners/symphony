...
An OPTIONAL HTTP interface for observability and operational control (dashboard +
`/api/v1/*` JSON API). It is an extension and is not REQUIRED for conformance.

See [`docs/spec/extensions/http-server.md`](docs/spec/extensions/http-server.md) for the
full extension contract, including config, enablement, endpoint shapes, and design notes.
...
An OPTIONAL extension profile in which Symphony keeps one central orchestrator but executes
worker runs on one or more remote hosts over SSH.

See [`docs/spec/extensions/ssh-worker.md`](docs/spec/extensions/ssh-worker.md) for the full
extension contract, including config, execution model, scheduling notes, and problems to
consider.
