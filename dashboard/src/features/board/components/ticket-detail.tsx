...
function issueOperations(ticket: Ticket, live?: LiveAgent): IssueOperation[] {
  ...
  const operations: IssueOperation[] = [
    ...
    },
    {
      ...
      primaryAction: "Rerun",
      description: "Clear the Symphony workspace and start a fresh agent attempt.",
      action: "rerun",
      targetIssueId: ticket.id,
      rows: [
        { label: "Run", value: running ? `Live, turn ${live?.turn ?? "unknown"}` : "Idle" },
        ...
        { label: "Will preserve", value: "Linear comments, PRs, and issue state" },
        { label: "Will queue", value: "Fresh Codex attempt if the issue is active" },
      ],
      note: running
        ? "This also terminates the active run before queueing the fresh attempt."
        : "Rerun clears only Symphony runtime/workspace state before queueing.",
