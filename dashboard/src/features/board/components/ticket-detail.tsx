...
function issueOperations(ticket: Ticket, live?: LiveAgent): IssueOperation[] {
  ...
  const operations: IssueOperation[] = [
    ...
    },
    {
      ...
      kind: "danger",
      risk: "destructive",
      summary: "Stop any active run, clear the workspace, and queue a fresh attempt.",
      primaryAction: "Rerun",
      description: "Move this issue back to the dispatch queue and start a fresh Codex attempt.",
      action: "rerun",
      targetIssueId: ticket.id,
      rows: [
        { label: "Run", value: running ? `Live, turn ${live?.turn ?? "unknown"}` : "Idle" },
        { label: "Target", value: ticket.id },
        { label: "Will delete", value: "Symphony workspace for this issue" },
        { label: "Will move", value: "Back to the dispatch queue" },
        { label: "Will preserve", value: "Linear comments and PRs" },
        { label: "Will queue", value: "A fresh Codex attempt" },
      ],
      note: running
        ? "This also terminates the active run before queueing the fresh attempt."
        : "Rerun works from any board state, including review: it moves the issue back to the dispatch queue and starts a fresh Codex attempt.",
      confirmation: "I understand this deletes the Symphony workspace for this issue.",
      backendReady: true,
    },
    {
      key: "stop",
