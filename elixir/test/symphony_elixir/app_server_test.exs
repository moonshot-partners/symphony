defmodule SymphonyElixir.AppServerTest do
    ...
    try do
      ...
      Enum.each(policy_cases, fn configured_policy ->
        ...
        write_workflow_file!(Workflow.workflow_file_path(),
          ...
          agent_runtime_turn_sandbox_policy: configured_policy
        )
  ...
  test "app server forwards devflow_context opt into thread/start params" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-devflow-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-devflow.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-devflow.trace}"
      count=0
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          1)
            ...
            ;;
          2)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-2001"}}}'
            ;;
          3)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-2001"}}}'
            ;;
          4)
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
      ...
      issue = %Issue{
        id: "issue-devflow",
        identifier: "MT-2001",
        title: "Forward devflow context to shim",
        description: "Ensure thread/start carries devflowContext when caller supplies it",
        state: "In Progress",
        url: "https://example.org/issues/MT-2001",
        labels: ["backend"]
      }

      devflow_root = Path.join(test_root, "lite")
      File.mkdir_p!(devflow_root)

      devflow_context = %{
        "issueIdentifier" => issue.identifier,
        "issueTitle" => issue.title,
        "issueUrl" => issue.url,
        "devflowRoot" => devflow_root,
        "policy" => "auto_deny",
        "basePrompt" => "You are an unattended Symphony agent."
      }

      assert {:ok, _result} =
               AppServer.run(workspace, "Validate devflow plumbing", issue, devflow_context: devflow_context)
               ...
               if String.starts_with?(line, "JSON:") do
                 ...
                 |> then(fn payload ->
                   payload["method"] == "thread/start" &&
                     get_in(payload, ["params", "devflowContext"]) == devflow_context
                 end)
               else
                 false
               end
  ...
  test "app server omits devflowContext from thread/start when opt absent" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-no-devflow-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-no-devflow.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-no-devflow.trace}"
      count=0
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          1)
            ...
            ;;
          2)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-2002"}}}'
            ;;
          3)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-2002"}}}'
            ;;
          4)
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
      ...
      issue = %Issue{
        id: "issue-no-devflow",
        identifier: "MT-2002",
        title: "Backwards-compat: no devflow context",
        description: "Ensure thread/start payload is unchanged when devflow_context opt is omitted",
        state: "In Progress",
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Validate backwards-compat", issue)
      ...
      assert is_binary(thread_start_line)
      ...
      refute Map.has_key?(payload["params"], "devflowContext")
    after
      File.rm_rf(test_root)
    end
  end
